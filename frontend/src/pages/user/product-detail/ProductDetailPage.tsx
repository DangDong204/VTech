import { CompareTray } from '@/components/user/compare/CompareTray'
...
export default function ProductDetailPage() {
  ...
  const { items: compareItems, addItem: addCompareItem } = useCompareStore()
  ...
  const handleCompare = () => {
    ...
    setIsCompareTrayOpen(true) // Khi bấm Thêm thì tự động mở Popup cho khách xem
  }
  ...
  return (
    <>
      <div className='bg-slate-50 min-h-screen pb-12 pt-4 relative'>
              ...
              <div className='md:col-span-5 flex flex-col'>
                ...
                <div className='border border-red-200 rounded-lg overflow-hidden mb-6 bg-white'>
                  ...
                  <div className='p-4 text-sm flex flex-col gap-3'>
                    ...
                    <div className='flex items-start gap-2'>
                      ...
                      <span>
                        {t(
                          'productDetail.promo2',
                          'Tặng Balo Laptop VTech cao cấp trị giá 300.000đ'
                        )}
                      </span>
                    </div>
                    <div className='flex items-start gap-2'>
                ...
                <div className='flex gap-2.5 mt-2'>
                  <button
                    ...
                    className='flex-1 border border-slate-300 text-slate-600 hover:bg-blue-50 hover:border-blue-300 hover:text-blue-600 rounded-lg py-2 flex items-center justify-center font-bold text-sm gap-2 transition-all'
                  >
                    <Scale className='w-4 h-4' /> {t('productDetail.compare')}
                  </button>
                </div>
        ...
        {compareItems.length > 0 && (
          <button
            onClick={() => setIsCompareTrayOpen(!isCompareTrayOpen)}
            className='fixed bottom-6 left-4 sm:left-6 z-40 bg-blue-600 text-white p-3.5 rounded-full shadow-xl shadow-blue-600/30 hover:bg-blue-700 transition-all hover:scale-110 flex items-center justify-center animate-in zoom-in duration-300'
            title='Mở khay so sánh'
          >
            <div className='relative'>
              <Scale className='w-6 h-6' />
              <span className='absolute -top-3 -right-3 bg-red-500 text-white text-[11px] w-5 h-5 flex items-center justify-center rounded-full font-bold border-2 border-white shadow-sm'>
                {compareItems.length}
              </span>
            </div>
          </button>
      ...
      {/* --- POPUP SO SÁNH --- */}
      <CompareTray isOpen={isCompareTrayOpen} onClose={() => setIsCompareTrayOpen(false)} />
    </>
  )
}
